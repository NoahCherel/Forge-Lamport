...
class Lamport:
    ...
    def generate_keys(cls) -> tuple[SecretKey, PublicKey]:
        ...
        for i in range(256):
            # Générer des pré-images aléatoires pour 0 et 1
            sk.zero_pre[i] = Block(secrets.token_bytes(32))
            sk.one_pre[i] = Block(secrets.token_bytes(32))

            # Calculer les hachages des pré-images
            pk.zero_hash[i] = Block(sk.zero_pre[i].hash_data())
            pk.one_hash[i] = Block(sk.one_pre[i].hash_data())
    ...
    def verify(cls, msg: Message, pk: PublicKey, sig: Sig) -> bool:
        ...
        for i in range(256):
            ...
            if bit == 0:
                expected = pk.zero_hash[i].data
            else:
                expected = pk.one_hash[i].data

            if not sig.preimage[i].is_preimage(expected):
                return False
