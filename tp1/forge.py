...
import tp1.signatures as signatures
...
def forge() -> tuple[str, Sig]:
    ...
    # Création d'une liste contenant les 5 signatures
    signatures1 = [sig_1, sig_2, sig_3, sig_4, sig_5]
    ...
    forged_msg = "Ce message est contrefait. NOCHE24@ulaval.ca "
    cpt = 0
    
    while True:
        msg_cpt = f"{forged_msg} {cpt}"
        msg = Message.from_str(msg_cpt)

        new_sig = Sig()

        try:
            for i in range(256):
                bit = msg.data[i // 8] >> (7 - (i % 8)) & 1
                found = False

                for sig in signatures1:
                    if not sig.preimage[i].is_empty():
                        expected_hash = pk.zero_hash[i].data if bit == 0 else pk.one_hash[i].data
                        if sig.preimage[i].is_preimage(expected_hash):
                            new_sig.preimage[i] = sig.preimage[i]
                            found = True
                            break

                if not found:
                    raise ValueError(f"Erreur forge")

            if Lamport.verify(msg, pk, new_sig):
                return msg_cpt, new_sig
